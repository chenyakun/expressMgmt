...
<script type="text/javascript">
	...
	function admin_buggl_editFun(id) {
		$('#admin_aboutgl_datagrid').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/aboutController/bugglEdit.action?id=' + id,
			width : 500,
			height : 450,
			modal : true,
			title : '编辑公告',
					...
					$('#admin_bugglEdit_editForm').form({
						...
						success : function(result) {
							try {
								...
								if (r.success) {
									$('#admin_aboutgl_datagrid').datagrid('updateRow', {
										index : $('#admin_aboutgl_datagrid').datagrid('getRowIndex', id),
										row : r.obj
									});
									d.dialog('destroy');
								}
					...
					$('#admin_bugglAdd_addForm').form({
						...
						success : function(result) {
							try {
								var r = $.parseJSON(result);
								if (r.success) {
									$('#admin_aboutgl_datagrid').datagrid('insertRow', {
										index : 0,
										row : r.obj
									});
									d.dialog('destroy');
			...
			$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.ajax({
						...
						success : function(result) {
							if (result.success) {
								$('#admin_aboutgl_datagrid').datagrid('load');
								$('#admin_aboutgl_datagrid').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
							}
							$.messager.show({
								title : '提示',
	...
	function admin_buggl_deleteFun(id) {
		$('#admin_aboutgl_datagrid').datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
		$('#admin_aboutgl_datagrid').datagrid('checkRow', $('#admin_aboutgl_datagrid').datagrid('getRowIndex', id));
