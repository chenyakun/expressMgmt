...
public class CountyDAO extends BaseHibernateDAO {
	...
	public County findById(java.lang.Integer id) {
		...
		try {
			County instance = (County) getSession().get("ex.model.County", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public County merge(County detachedInstance) {
		...
		try {
			County result = (County) getSession().merge(detachedInstance);
